'''
改DAPPM，减少一层残差结构，在总和的最后一层残差结构加入注意力机制
'''
...
class DAPPM(nn.Layer):
    ...
    def forward(self, x):
        ...
        # NCHW
        out = self.compression(self.atten(paddle.concat(x_list, 1))) + self.shortcut(x)
...
model = DDRNet_23(num_classes=19)
print(model)

intput_shape = (2,3,1024,1024)
intput_data = paddle.randn(intput_shape)
